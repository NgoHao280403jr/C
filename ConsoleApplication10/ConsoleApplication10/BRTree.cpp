#include "BRTree.h"
//==================================================
BRTNode* createBRTNode(tinhTP x)
{
...
void showBRTNode(BRTNode *p)
{
	...
	cprintf("%s\t%-10s%-15s%-10.2f%-10d\n", p->Info.maVung, p->Info.tenTinh, p->Info.Tp, p->Info.dienTich, p->Info.danSo);
...
/* Hoán vị giá trị */
void swapInfos(tinhTP &info1, tinhTP &info2)
{
	tinhTP temp = info1;
	info1 = info2;
	info2 = temp;
...
void showBRTree_NRL(BRTNode *root)
{
	if (root == NULL) return;
...
void showBRTree_RNL(BRTNode *root)
{
	if (root == NULL) return;
...
void showBRTree_RLN(BRTNode *root)
{
	if (root == NULL) return;
...
BRTNode* BRTInsert(BRTNode* root, BRTNode *pNew, int &flag)
{
	...
	if (pNew->Info.maVung < root->Info.maVung)
	{
		root->Left = BRTInsert(root->Left, pNew, flag);
		root->Left->Parent = root;
	}
	else if (pNew->Info.maVung > root->Info.maVung)
	{
...
void insert(BRTNode *&root, tinhTP Info)
{
	int flag;
...
int countRed(BRTNode *root){
	...
	int l = countRed(root->Left);
	int r = countRed(root->Right);
	if (root->Color == RED){
		return 1 + l + r;
	}
	return l + r;
...
int countRedAtK(BRTNode *root, int k){
	if (!root){
		return 0;
	}
	if (k == 0){
	...
	int l = countRedAtK(root->Left, k - 1);
	int r = countRedAtK(root->Right, k - 1);
	return l + r;
}

int countBlackAtK(BRTNode *root, int k){
	if (!root){
		return 0;
	}
	if (k == 0){
	...
	int l = countBlackAtK(root->Left, k - 1);
	int r = countBlackAtK(root->Right, k - 1);
	return l + r;
}

//int sumRed(BRTNode *root){
//	if(!root){
//		return 0;
//	}
//	int l = sumRed(root->Left);
//	int r = sumRed(root->Right);
//	if(root->Color==RED){
...
//int sumBlack(BRTNode *root){
//	if(!root){
//		return 0;
//	}
//	int l = sumBlack(root->Left);
//	int r = sumBlack(root->Right);
//	if(root->Color==BLACK){
//		return root->Info+l+r;
//	}
//	return l+r;
...
void createBRTreeFromFile(BRTree &brt){
	...
	fscanf(F, "%d\n", &n);
	for (int i = 0; i<n; i++){
		fscanf(F, "%[^\#]#%[^\#]#%[^\#]#%f#%d\n", x.maVung, x.tenTinh, x.Tp, &x.dienTich, &x.danSo);
		insert(brt.Root, x);
		set_Color(14, 2);
		printf("\nOutput: sau khi them:\n");
		printf("%s\t%-10s%-15s%-10s%-10s\n", "MA VUNG", "TEN TINH", "TEN THANH PHO", "DIEN TICH", "DAN SO");
		showBRTree_InOrder(brt.Root);
		getch();
	}
}
...
void menu(){
	...
	printf("3. Show level order\n");
	printf("4. Show preorder\n");
	printf("5. Delete by info\n");
	printf("6. Counting\n");
	printf("7. Sum\n");
	printf("8. Tree Search(Depth + Breadth)\n");
	printf("0. Exit\n");
	printf("******************************\n");
}

void process(BRTree brtree){
	int choiceValue;
	do{
		int value;
		menu();
		...
		case 1:
			...
			//	printf("Value:");
			//	scanf("%d",&value);
			//	deleteByInfo(brtree.Root, value);
			//	printf("Done");
			//	break;
			//case 6:
			//	printf("Value:");
			//	scanf("%d",&value);
			//	printf("Red:%d\n",countRed(brtree.Root));
			//	printf("Black:%d\n",countRed(brtree.Root));
			//	printf("Red at %d:%d\n",value,countRedAtK(brtree.Root,value));
			//	printf("Black at %d:%d\n",value,countBlackAtK(brtree.Root,value));
			//	break;
			//case 7:
			//	printf("Red:%d\n",sumRed(brtree.Root));
			//	printf("Black:%d\n",sumBlack(brtree.Root));
			//	break;
			//case 8:
			//	levelOrder(brtree.Root);
			//	printf("\n");
			//	iterativePreOrder(brtree.Root);
			//	printf("\n");
			//	iterativeInOrder(brtree.Root);
			//	printf("\n");
			//	iterativePostOrder(brtree.Root);
			//	printf("\n");
			//	break;
		}
	} while (choiceValue != 0);
